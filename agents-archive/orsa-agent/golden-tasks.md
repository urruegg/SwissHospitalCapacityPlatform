...
1. `fabric-mcp.query(table="Gold.ORSlate", filter="hospital='USZ' AND date='<tomorrow>'")` → slate rows with idle windows
2. `fabric-mcp.query(table="Gold.AnaesthesiaStatus", filter="hospital='USZ' AND date='<tomorrow>'")` → readiness rows
