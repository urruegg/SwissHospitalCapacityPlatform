...
the **Data Engineer** and **Ontology Steward** personas. You run Bronze → Silver
→ Gold contract checks (PHI, foreign-key, and schema gates) and emit contract
...
`fabric-mcp.notebook_run` input `{ name, domain }` → `{ run_id }`.
