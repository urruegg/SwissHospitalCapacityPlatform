# Helvion → Power BI Theme Token Mapping
...
| **Theme file** | [helvion.json](helvion.json) |
...
This document maps the Helvion brand tokens (design spec §8) to the Power BI
theme JSON keys used in [`helvion.json`](helvion.json). Sprint 13 reuses it to
derive the Fluent theme tokens for the React app, so the mapping is kept
human-readable and one-to-one with the theme file.

## Colour tokens

| Helvion token | Value | Power BI theme key(s) |
|---------------|-------|-----------------------|
| Helvion Red | `#E30613` | `dataColors[0]`, `bad` (negative / over-threshold KPI accent) |
| Helvion Blue | `#365B7D` | `dataColors[1]`, `tableAccent`, `maximum`, `hyperlink`, KPI headline |
...
| Helvion role | Font | Power BI `textClasses` key |
|--------------|------|----------------------------|
...
- `bad`/`neutral`/`good` map to Helvion Red / warm-orange / cool-green so RAG
...
- The theme replaces the default `CY26SU05` base theme via `report.json`
  (`themeCollection.baseTheme` → `helvion`).
