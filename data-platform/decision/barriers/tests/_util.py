"""Dependency-free JSON-Schema (draft-07 subset) validator for the barrier
Gold-materialization offline tests.

Mirrors the stdlib-only validator in the Foresight tests
(``data-platform/notebooks/foresight/tests/_util.py``) so the barrier
schema-conformance test runs identically in CI and locally without
``jsonschema``. Supports exactly the keywords used by the barrier contract:
``type`` (incl. union lists), ``required``, ``properties``,
``additionalProperties: false``, ``enum``, ``const``, ``items``, ``minItems``,
``minimum``, ``minLength``, ``pattern`` and the ``date-time`` ``format``
(structural only).
"""
