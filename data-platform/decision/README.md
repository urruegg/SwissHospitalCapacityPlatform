...
| **Live-apply tooling (WS-C)** | [`coordination/cosmos_store.py`](coordination/cosmos_store.py) + [`coordination/seed_live.py`](coordination/seed_live.py) + [`foundry/register_decision_tier.py`](foundry/register_decision_tier.py) | `CosmosStore(PlanStore)` (RBAC-only, no keys) over the `plans`/`proposed_actions` containers; a HITL-gated live seed replaying all six roles (`--action plan` dry run / `--action apply --approved-to-apply <handle>`); and a Foundry decision-tier registration tool for the six agents (same plan/apply gate). The SIT Cosmos account is private-endpoint only ([ADR-0029](../../docs/adr/0029-agent-host-cosmos-reachability.md)) and Foundry is eastus2 ([ADR-0032](../../docs/adr/0032-foundry-control-plane-eastus2.md)), so a real apply runs in-VNet, never from CI. |
