...
describe('DischargeBarriersBoard', () => {
  it('renders barrier labels and bed-impact numbers', () => {
    render(
      <FluentProvider theme={webLightTheme}>
        <DischargeBarriersBoard barriers={DISCHARGE_PINNED.barriers} onSelectBarrier={vi.fn()} />
      </FluentProvider>,
    );
    const sorted = [...DISCHARGE_PINNED.barriers].sort((a, b) => b.bedImpact - a.bedImpact);
  ...
  it('renders barriers sorted by bedImpact descending even when input is unsorted', () => {
    ...
    render(
      <FluentProvider theme={webLightTheme}>
        <DischargeBarriersBoard barriers={reversed} onSelectBarrier={vi.fn()} />
      </FluentProvider>,
    );
    const sorted = [...DISCHARGE_PINNED.barriers].sort((a, b) => b.bedImpact - a.bedImpact);
  ...
  it('fires onSelectBarrier with the correct barrier when a row is clicked', () => {
    ...
    render(
      <FluentProvider theme={webLightTheme}>
        <DischargeBarriersBoard barriers={DISCHARGE_PINNED.barriers} onSelectBarrier={onSelectBarrier} />
      </FluentProvider>,
    );
    const sorted = [...DISCHARGE_PINNED.barriers].sort((a, b) => b.bedImpact - a.bedImpact);
