...
<#
.SYNOPSIS
    Resume a Fabric F2 capacity (DX.2, Sprint 09 v2.0.0).

.DESCRIPTION
    Idempotent — checks current state before invoking resume. Returns 0 on success or if already Active.
    Uses proven Sprint 00 pattern: `az resource invoke-action --action resume` on the capacity resource ID.
...
.EXAMPLE
    .\Resume-FabricCapacity.ps1 -Environment sit
...
.NOTES
    ...
    Tests:   infra/scripts/tests/Resume-FabricCapacity.Tests.ps1
#>
...
try {
    ...
    if ($current -and $current.properties -and $current.properties.state -eq 'Active') {
        Write-Host "[NO-OP] Capacity is already Active." -ForegroundColor Green
        exit 0
    }
    $observedState = if ($current -and $current.properties) { $current.properties.state } else { 'Unknown' }
    Write-Host "Current state: $observedState -- invoking resume..."
}
catch {
    Write-Warning "Could not read current state: $_. Proceeding with resume anyway."
}

# Invoke resume
try {
    $result = az resource invoke-action --ids $capacityId --action resume 2>&1
    Write-Host "[OK] Resume action invoked." -ForegroundColor Green
    if ($result) { Write-Host "Response: $result" }
    exit 0
}
catch {
    Write-Error "Resume failed: $_"
    exit 3
}
