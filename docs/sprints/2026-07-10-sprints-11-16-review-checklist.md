...
| **Author** | Urs Rüeegg |
| **Status** | Reviewed |
| **Previous Version** | 1.10.0 (day-1 fix-pass close with S12.4 partial + 2 fresh sub-gaps); this bump captures **day-2 chunk #4c-e results**: S12.4 fully closed — (i) `gh-oidc-ihzhhpf` granted Workspace Contributor on `ws-ihzhhpf-sit-data` (Fabric REST `POST /roleAssignments` HTTP 201); (ii) notebook lakehouse-binding added via `updateDefinition` API (`metadata.dependencies.lakehouse`); (iii) Cell 2 rewritten from Synapse `spark.synapse.linkedService` → Fabric-native `azure-kusto-data.KustoClient` + LAW ADE endpoint + `notebookutils.credentials.getToken('kusto')` audience (after ruling out `azure-monitor-query` + `DefaultAzureCredential` which Fabric does not support); (iv) `adoption_transforms.py` inlined into the notebook (Fabric can't import repo-local .py modules); (v) `env-adoption-kusto` Fabric Environment (id `4d8ad472-8523-472a-91f9-770e1bfc76c3`) authored + published with `azure-kusto-data==5.0.5`; (vi) Log Analytics Reader granted to `gh-oidc-ihzhhpf` on `log-ihzhhpf-sit`; (vii) end-to-end notebook run **Completed** (2026-07-15T07:25:28Z, 5min 30s, 0 real sign-in rows fetched — expected in demo tenant, Bronze write path exercised). |
=======
| **Version** | 1.10.0 |
| **Date** | 2026-07-14 |
| **Author** | Urs Rüeegg |
| **Status** | Reviewed |
| **Previous Version** | 1.9.0 (2026-07-14 pm evidence refresh with Fabric workspace inventory); this bump captures **day-1 findings from the fix-pass session** (chunk #4a-b): adoption notebook published to Fabric (id `d4771009-...`); HTTP 411 in Fabric jobs POST fixed in workflows (PR #221); Fabric API contract quirks documented (`format: ipynb` required); two NEW sub-gaps discovered — (i) missing `.platform` lakehouse binding on the published notebook causes runtime failure `System_Cancelled_Session_Statements_Failed`, (ii) workflow's federated identity lacks Fabric workspace membership (HTTP 403 `InsufficientPrivileges`). BVA chunk #4d + S16.5 not started today; queued for tomorrow. |
>>>>>>> origin/main
...
| **Overall** | **53** | **4** | **5** | **7** | **2** | **71** |

**Green rate: 75%** (was 73% on 2026-07-14). Of the 6 remaining red/partial-red items, **5 collapse to 2 root causes** — none of them are Sprint 11 or PBI issues, both of which are fully green on code-verifiable criteria. (The former third root cause — Sprint 14.1 — was closed on 2026-07-13; Sprint 13.1 partial S13.6 closed on 2026-07-14; **S12.4 closed on 2026-07-15**.)
...
=======
| S12.4 | Adoption telemetry pipeline emitting nightly files within 24h of T5 merge. | ⚠️ **UPDATED 2026-07-14 (day-1 fix-pass close)** — partial (progress this session, 2 fresh sub-gaps discovered) | **Progress today:** (i) Notebook published to `ws-ihzhhpf-sit-data` via Fabric REST API (`POST /items?format=ipynb`, id `d4771009-09c1-48f8-8c15-919d88993f2e`); (ii) `FABRIC_WORKSPACE_ID` + `FABRIC_ADOPTION_NOTEBOOK_ID` GH env variables set on `sit` env; (iii) HTTP 411 in `adoption-refresh.yml`'s `curl -X POST` fixed via PR #221 (added `--data '{}'`). **Fresh sub-gaps found:** (iv) Notebook runtime fails with `System_Cancelled_Session_Statements_Failed` because Fabric API publish does NOT auto-attach the target lakehouse — the notebook needs a `.platform` metadata file in the definition parts declaring `default_lakehouse: lh_ihzhhpf_sit`. PR #186's authored `.ipynb` has no such metadata; Fabric portal auto-adds it on UI import. (v) Workflow's federated identity (`AZURE_CLIENT_ID` from env `sit`) gets HTTP 403 `InsufficientPrivileges` on the Fabric jobs POST — the identity is not a member of `ws-ihzhhpf-sit-data`. | **Remaining sub-gaps:** (1) republish notebook with `.platform` metadata OR attach the lakehouse via Fabric portal clickops (recommend the former for repeatability); (2) grant the federated identity Workspace Contributor role on `ws-ihzhhpf-sit-data` (Fabric REST API `POST /workspaces/{id}/roleAssignments` OR portal); (3) same env-gate governance decision still open. Tracked as [issue #180](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/180). |
>>>>>>> origin/main
...
**Sprint 12 result (revised 2026-07-15 am after day-2 fix-pass close of S12.4): 7/10 ✅ done, 1 ⚠️ partial (S12.6 CI trigger re-verification), 2 ⏳ audit-pending (unblocked, awaiting Sprint 13.1 recovery deploy). 0 ❌ gap.**
...
- **S12.4 (closed 2026-07-15):** adoption notebook now runs end-to-end in Fabric via `env-adoption-kusto` Environment + Fabric-native Kusto client pattern. Followed by S12.6 workflow CI re-verification (non-blocking).
- **S12.6 (in flight):** the `adoption-refresh.yml` workflow uses the same identity as day-2 verification; re-run needed to prove the CI trigger path end-to-end. Tracked via [issue #180](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/180).
