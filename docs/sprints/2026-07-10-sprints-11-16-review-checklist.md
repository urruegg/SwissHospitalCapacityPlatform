...
| **Author** | Urs Rüeegg |
| **Status** | Reviewed |
| **Previous Version** | 1.17.1 (chunk #4n — S15.4 promotion + header hygiene). This bump records the **#181 GitHub-issue close-out (2026-07-18)**: the last residual S13.6 note (interactive Copilot Drawer click-test) is now ✅ proven live — the deployed `hcc-app-fluent` calls the live agent-host (VITE build-arg + CORS), browser-style `POST` returns a grounded reply (corr `77960709b80ebf57`) + PHI refusal (corr `969eaf364470da54`), evidenced in [`docs/architecture/fabric-iq-ready-evidence.md`](../architecture/fabric-iq-ready-evidence.md) v1.2.0. |
