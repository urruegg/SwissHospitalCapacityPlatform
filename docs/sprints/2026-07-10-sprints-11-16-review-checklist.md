...
| **Author** | Urs Rüeegg |
| **Status** | Reviewed |
| **Previous Version** | 1.16.0 (day-2 chunk #4j close of S16.3 via Path A Fabric MPE notebook). This bump captures the **review close-out session (chunks #4k + #4l + #4m)** combined into a single PR: (i) **#4k audit-doc hygiene** — pre-existing overall rollup drift `58/2/3/6/2` did not match the per-row sum `59/2/0/8/2`; corrected + green-rate refresh 82% → 83% → 86% after #4l closures; (ii) **#4l interactive Playwright smoke** against the running SIT app shell `ca-app-fluent-ihzhhpf-sit.ashysky-8f51a689.westus2.azurecontainerapps.io` closed **S12.3** (via dry auth callback: CSA workspace role gate + Backstage Rollen Graph live query both prove server-side identity + role-check pipeline works) and **S13.5** (Rollen & RBAC tab renders live app-role table `Role | Value | Assignments` with rows from `Directory.Read.All` on the app registration's SP); the smoke also refreshed the rationale for **S15.4** (blockers S14.4/S13.1 are stale — both closed 2026-07-13/14; the real gap is a small app-code follow-up to add BVA cards to the Evidence tab whiteboard projection, DOM inspection returned zero `bva-card-*` entries despite the 3 cards being registered in `CardRegistry.tsx`); (iii) **#4m Fabric data-plane check** for S14.2 confirmed the JSON publish cycle (`evidence-publish.yml`) runs green (5/5 recent success) but the 5 evidence medallion notebooks under `data-platform/notebooks/evidence/` (`ingest_bronze`, `build_silver`, `build_gold_dims`, `build_gold_facts`, `score_readiness`) are not published to Fabric — explicit Sprint 17 T1 scope per the notebook README `deploy`-ceiling gate. Net effect: **+2 done, -2 audit-pending**, rollup `59/2/0/8/2` → `61/2/0/6/2`, green rate 83% → 86%.
| **Date** | 2026-07-15 |
| **Author** | Urs Rüeegg |
| **Status** | Reviewed |
| **Previous Version** | 1.15.0 (day-2 chunk #4i close of S16.5 via CSA wizard skeleton). This bump captures **day-2 chunk #4j — S16.3 close via Path A (Fabric MPE notebook)**: (i) new durable artefact `data-platform/notebooks/csa/csa-seed-scenarios.ipynb` published to Fabric workspace `f3af9733-9503-4e92-98f9-a901d96f1c87` (notebook id `3978b53c-087d-4aab-8dc1-a38e4303610f`) and executed 2026-07-15 15:56 CET with final status `Completed` / livy state `Succeeded`; the notebook's assert `expected 8 scenarios in Cosmos` passed, proving all 8 scenarios are now upserted into `cosmos-csa-ihzhhpf-sit / csa / scenarios`; (ii) new authoring script `.scratch/publish_and_run_seed_scenarios.py` (gitignored per repo convention) that regenerates the notebook from `data/csa/scenarios/*.yaml` — fixed a Python-literal bug where `json.dumps` was emitting JSON booleans (`false`/`true`/`null`) into a Python source cell, causing the initial `NameError: name 'false' is not defined`; now uses `pprint.pformat` for Python-native literals; (iii) MG-policy blocker `CosmosDB_PublicNetwork_Modify` documented + tracked as [closed issue #233](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/233) (out of scope for MCAPS demo tenant; revisit in production tenant); (iv) merged workflow [PR #232](https://github.com/urruegg/SwissHospitalCapacityPlatform/pull/232) `csa-scenario-sync.yml` retained as pre-built long-term CI path once a self-hosted runner lands. **Previous version's** #4i content preserved: new CSA wizard component tree at `apps/hcc-app-fluent/src/workspaces/main/wizards/csa/` — `CsaWizard.tsx` + `CsaRoleGuard.tsx` + `CsaStepper.tsx` + `CsaStepBody.tsx` + `csa-steps.ts`; role gate enforced against `HCC.CrisisManager` / `HCC.OperationsLead` / `HCC.PlatformAdmin` / `HCC.SuperAdmin`; 4-step linear stepper wired to `csa-agent`; 10 vitest unit tests. Mini-sprint [#229](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/229) closed same day. (i) new component tree at `apps/hcc-app-fluent/src/workspaces/main/wizards/csa/` — `CsaWizard.tsx` + `CsaRoleGuard.tsx` + `CsaStepper.tsx` + `CsaStepBody.tsx` + `csa-steps.ts`; (ii) role gate enforced against `HCC.CrisisManager` / `HCC.OperationsLead` / `HCC.PlatformAdmin` / `HCC.SuperAdmin` (per S16 spec §8); (iii) 4-step linear stepper (Prepare → Run → Evaluate → Recommend) with progress indicator; Prepare is **wired live** to `csa-agent` chat via `useAgentInvoker`; Run/Evaluate render stub scaffolds with explicit "Sprint 13 T5 MCP-wiring pending" badges; Recommend renders read-only sample view over the 3 recommendation PRs shipped during Sprint 16 T4; (iv) Copilot Drawer opens with `agent="csa-agent"` in the right rail per spec §8; (v) new rail entry "CSA" (`WorkspaceKey = 'csa'`) with i18n keys in `en.json` + `de.json`; (vi) 10 vitest unit tests covering pure step module + role guard + App integration. Mini-sprint [#229](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/229) closed same day. |
...
| **Sprint 15** — BVA | 7 | 1 | 0 | 0 | 0 | 8 |
| **Sprint 16** — CSA | 10 | 1 | 0 | 0 | 1 | 12 |
| **PBI Demoable v2** — M2-M6 | 9 | 0 | 0 | 4 | 0 | 13 |
| **Overall** | **61** | **2** | **0** | **6** | **2** | **71** |

**Green rate: 86%** (was 83% pre-close-session 2026-07-15). Only 2 partial items remain (S15.4 needs a small app-code follow-up to wire BVA cards into the Evidence whiteboard projection; S16.4 needs Sprint 13 T5 MCP-wiring completion) plus 6 audit-pending items (S12.5 blocked on PROD deploy #179; S14.2 blocked on Sprint 17 T1 Fabric medallion publish; PBI.2 + PBI.6 + PBI.10 + PBI.11 blocked on PBI Desktop / Fabric CLI availability). Root cause history: Sprint 14.1 closed 2026-07-13; Sprint 13.1 partial S13.6 closed 2026-07-14; **S12.4 + S12.6 + S13.9 + S15.1 + S16.5 closed on 2026-07-15 (chunks #4a–#4i); S16.3 closed on 2026-07-15 (chunk #4j) via Path A Fabric MPE notebook; S12.3 + S13.5 closed on 2026-07-15 (chunks #4k–#4m) via interactive Playwright smoke against SIT + audit-doc hygiene pass**.
...
Break-down after chunk #4l + #4m close-out session:

- **App-shell running — remaining smoke tasks against SIT** (`https://ca-app-fluent-ihzhhpf-sit.ashysky-8f51a689.westus2.azurecontainerapps.io`): S12.5 (env-scoping smoke; blocked on PROD deploy [issue #179](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/179) — SIT-only is intrinsically single-env) + S15.4 (⚠️ partial — needs small app-code follow-up to wire BVA cards into Evidence whiteboard projection).
...
2. **Sprint 13 T5 (MCP-wiring completion)** — closes S16.4 partial by wiring `fabric-mcp.run-notebook` + `cosmos-mcp.vector-query` + real Foundry chat model.
3. **Sprint 17 mini-scope (BVA whiteboard projection)** — closes S15.4 partial with a small `apps/hcc-app-fluent/src/workspaces/backstage/tabs/evidence/EvidenceTab.tsx` follow-up that adds the 3 registered BVA card ids to the whiteboard's card-selection projection.
4. **PROD promotion** — [issue #179](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/179) closes S12.5 (env-scoping smoke needs two envs). Tenant-migration-ready CSVs already shipped via PR #185.
...
| S15.4 | BVA card cluster visible on Sprint 14 presenter whiteboard (BVA filter/tab). | ⚠️ **UPDATED 2026-07-15 (chunk #4l)** — partial (rationale corrected) | 3 BVA cards defined and registered: `apps/hcc-app-fluent/src/cards/bva/{BvaHeadlineKpiCard, BvaPlanVsActualCard, BvaTrendCard}.tsx` → imported by `CardRegistry.tsx` lines 9-11, 29-31. **Runtime confirmation via interactive Playwright smoke against `ca-app-fluent-ihzhhpf-sit.*/#/backstage`** on 2026-07-15: Evidence tab (`Nachweise`) renders the presenter whiteboard with parity KPI pills (T-SHOW 100%, T-PROD 84%, GA-Paritätslücke: 4) + **25 BOM** cards (matches `bom-card-bom-*` DOM ids) + **33 ADR** cards (satisfies S14.4 ≥10 ADR DoD). Total 229 whiteboard cards rendered end-to-end. **However**: DOM inspection returned **zero** `bva-card-*` or `[class*="bva"]` entries, meaning the 3 BVA cards from CardRegistry are **not wired into either whiteboard tab** (`CH Nord × T-SHOW` or `GA-Paritätsansicht`). Blockers previously named (S14.4, S13.1) are stale (both closed 2026-07-13/14); the **real gap** is a small app-code follow-up that adds a BVA filter or tab entry to the Evidence tab and includes the 3 registered BVA card ids in that view's card-selection projection. | **Small app-code follow-up** in `apps/hcc-app-fluent/src/workspaces/backstage/tabs/evidence/EvidenceTab.tsx` (or the card-selection module it delegates to): add a BVA card cluster to the whiteboard projection so `BvaHeadlineKpiCard` + `BvaPlanVsActualCard` + `BvaTrendCard` render. Not blocking any Sprint 16 close-out. Recommend rolling into a Sprint 17 mini-scope batched with other whiteboard refinements. |
...
**Sprint 15 result (revised 2026-07-15 pm after day-2 chunk #4h close of S15.1): 7/8 ✅ done, 1 ⚠️ partial (S15.4 blocked by S14.4), 0 ❌ gap. Previous ❌ S15.1 closed via monolithic Fabric notebook + workflow pivot to jobType=RunNotebook.**

**Gaps requiring gap-fill PRs:**

- **S15.1 (closed 2026-07-15):** monolithic `bva_medallion_ingest` notebook published + workflow pivoted to jobType=RunNotebook. Mini-sprint [#225](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/225).
