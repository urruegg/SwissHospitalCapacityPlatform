...
| **Author** | Urs Rüeegg |
| **Status** | Reviewed |
| **Previous Version** | 1.12.0 (day-2 chunk #4f with S12.6 close-out); this bump captures **day-2 chunk #4g — CSA + HITL runtime audit refresh via live probe of `ca-agent-host-ihzhhpf-sit`**: (i) **S13.9 closed ⏳ → ✅** — all 5 HITL gates (HITL-01..05) validated end-to-end via runtime probe against the deployed agent-host: deny-by-default with no evidence returns HTTP 403 (deny), full 8-field ADR-0007 evidence schema (`gateId`/`approverObjectId`/`approverRole`/`decisionTimestampUtc`/`correlationId`/`decisionContextHash`/`decisionOutcome`/`sourceWorkflow`) on both HITL-01 + HITL-04 returns `{decision:'allow', gateId:'HITL-04'}` for `csa-agent`'s `vector-query` tool; (ii) **S16.4 evidence refreshed** — `csa-agent` manifest confirmed loaded in the deployed agent-host (`GET /agents` returns 7 Sprint 11 agents including CSA); chat + tool endpoints reachable and correctly gated; live LLM + real MCP tool wiring remains Sprint 13 T5 stub-mode (`MockChatModel` for chat, `{decision:'allow', tool}` stub for tool endpoints — the 3 recommendation PRs in `docs/csa/runs/` remain the only physical evidence of full Prepare/Run/Evaluate/Recommend); (iii) [issue #225](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/225) opened for the S15.1 mini-sprint (BVA medallion notebooks + Data Factory Pipeline publish — analog to closed #180). |
...
| **Overall** | **55** | **3** | **5** | **6** | **2** | **71** |

**Green rate: 77%** (was 76% mid-day 2026-07-15, 75% am 2026-07-15, 73% 2026-07-14). Of the 5 remaining red items and 3 partials, **6 collapse to 2 root causes** — none of them are Sprint 11 or PBI issues, both of which are fully green on code-verifiable criteria. (The former third root cause — Sprint 14.1 — was closed on 2026-07-13; Sprint 13.1 partial S13.6 closed on 2026-07-14; **S12.4 + S12.6 + S13.9 closed on 2026-07-15**.)
