...
### Sprint 14.1 retro notes (mini-sprint — Evidence tab T4–T6)

- **Closes the Sprint 14 deferred T4–T6 gap** (issue #182): the Fluent app now
  exists on `main`, so the whiteboard Evidence tab and its semantic-model backing
  were buildable at `write` ceiling.
  - **ADR** — [`../adr/0026-evidence-readiness-measure-ownership.md`](../adr/0026-evidence-readiness-measure-ownership.md)
    records the readiness-measure ownership decision: **Option B** — a dedicated
    `evidence.SemanticModel` (separate consumer from `capacity-dashboard`, cleaner
    RLS boundary, avoids the `verify-semantic-model.yml` contract gate).
  - **T4** — `data-platform/reports/evidence.SemanticModel/` Direct Lake TMDL over
    `gold.fact_readiness_snapshot` + `gold.fact_readiness_summary` with 5 readiness
    measures (BOM count, Readiness % T-SHOW / T-PROD, GA-Parity Gap, Blocked count)
    and one read-only role; 5 structure tests. Fabric publish still gated on S17 T1
    Git integration.
  - **T5** — 5 evidence card types (`BomCard`, `AdrCard`, `PrdRequirementCard`,
    `GaEvidenceCard`, `DependencyEdge`) + a shared provenance footer whose contract
    **fails render** (visible `role="alert"`) when `sourceUrl`/`asOf` is missing.
    App fixture generated byte-stable from the seed catalogs by
    `scripts/evidence/build_app_fixture.py`.
  - **T6** — `apps/hcc-app-fluent/src/workspaces/backstage/tabs/evidence/EvidenceTab.tsx`
    renders ≥ 25 BOM + ≥ 10 ADR + ≥ 1 PRD-requirement cards + dependency edges with
    catalog / GA-parity presets. 35 vitest tests + 2 Playwright E2E specs green;
    production build clean.
- **Still gated** (out of scope here): Fabric semantic-model publish (S17 T1),
  SIT app re-deploy (S13.1). All source + docs authored and verifiable now.
