# Sprint 29 — Foundry IQ Context Architecture — design
...
| **Status** | Approved for delegation (Sprint 29) |
| **Previous Version** | 1.0.0 (draft on branch `sprint-27/curavias-ux-polish` as `2026-07-26-foundry-iq-context-architecture-design.md`; relocated + renumbered to Sprint 29) |
| **Applies to** | `apps/hcc-app-fluent` context model + Foundry IQ / Fabric IQ integration |
| **Governing issue** | [#399 — Sprint 29 Foundry IQ context architecture](https://github.com/urruegg/SwissHospitalCapacityPlatform/issues/399) |
...
> per-user + per-agent + per-board context, consistently."* This spec governs
> **Sprint 29**, a dedicated follow-on sprint. It is design-only; no code lands
> from this doc — implementation is driven by
> [`2026-07-26-sprint-29-foundry-iq-context-architecture.md`](../plans/2026-07-26-sprint-29-foundry-iq-context-architecture.md).
...
plus grounding are enforced through the envelope — all **config-gated** so it
lifts from the westus2 demo (simulated) to live SIT without code edits.
...
Sprint 29 is delegated to **one dedicated worktree + Copilot CLI session** using
`superpowers:subagent-driven-development` (one milestone/task at a time, TDD,
frequent commits). See
[`docs/runbooks/sprint-29-worktree-delegation.md`](../../runbooks/sprint-29-worktree-delegation.md).
The app work is concentrated in a single codebase (`apps/hcc-app-fluent`) with a
mostly-sequential milestone chain (M0 → M1/M2 → M3 → M4 → M5 → M6), so a single
worktree avoids cross-worktree merge churn while the in-session subagent pattern
keeps each milestone isolated and test-first.
