# Sprint 28 - Curavias Product Owner Agent: full build (Design)
...
| **Sprint** | Sprint 28 - Curavias Product Owner Agent full build (tracked by the Sprint 28 GitHub issue) |
...
> **For agentic workers:** This is the approved design for Sprint 28. The
> implementation plan is produced separately via `superpowers:writing-plans`
> ([`2026-07-25-sprint-28-product-owner-agent.md`](../plans/2026-07-25-sprint-28-product-owner-agent.md))
> and delegated per-workstream to parallel Copilot CLI worktrees via
> [`docs/runbooks/sprint-28-worktree-delegation.md`](../../runbooks/sprint-28-worktree-delegation.md).
...
| -- | ---- | ------------------------- | ---------- | ------ |
| **G0** Governance and agent pack | ADR(s), `agents/product-owner-agent/` pack, AGENTS.md row, PRD FR/NFR + Section 7, MCP allow-list review, additional-domain registration pattern | Additional-domain registration contract | - | `sprint-28/ws-g0-governance` |
| **INF** Infra (Bicep) | Azure AI Search + Foundry IQ knowledge base, ADLS corpus landing + OneLake shortcut, Container Apps runtime + daily refresh job, Cosmos audit store, Key Vault, RBAC, Azure OpenAI deploy | Extra knowledge-source scaffolding for Compliance/Data/Ops domains | G0 contracts | `sprint-28/ws-inf-bicep` |
| **A** Class A corpus | daily GitHub -> ADLS -> OneLake -> knowledge source, chunk/tag, PHI gate, interviews first-order | source-language tagging (DE/EN); Compliance/Data/Ops corpora | INF | `sprint-28/ws-a-corpus` |
| **B** Class B live-proof | read-only probes + reconcile-and-flag; the 5 reference questions | - | G0 | `sprint-28/ws-b-liveproof` |
| **C** Class C cost | Cost Management (PROD) + Copilot token cost + BVA reconciliation | - | G0 | `sprint-28/ws-c-cost` |
| **D** Class D ontology | wire `da_hospital_capacity`; concept + gold-binding citations | - | G0 | `sprint-28/ws-d-ontology` |
| **X** Experience rail | START + BACKSTAGE Copilot rail; MAIN-board pattern | partner-scoped surface variant; UI DE/EN | G0 | `sprint-28/ws-x-rail` |
| **RT** Runtime + eval + RAI | orchestrator, grounded-answer contract, authz-aware retrieval, audit logging, per-persona golden-question harness, transparency banner, injection defence | multilingual DE/EN answering; partner entitlement tier | A/B/C/D contracts | `sprint-28/ws-rt-runtime` |
