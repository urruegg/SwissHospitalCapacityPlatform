# Sprint 27 - Worktree Delegation Runbook (parallel Copilot CLI)
...
> **Purpose:** package the 8 Sprint 27 workstreams so each can be built **in
> parallel** by a fresh Copilot CLI agent in its own isolated git worktree.
> Grounded in the [Sprint 27 design](../superpowers/specs/2026-07-25-sprint-27-product-owner-agent-design.md)
> and [plan](../superpowers/plans/2026-07-25-sprint-27-product-owner-agent.md).
...
git worktree add ".worktrees/$ws" -b "sprint-27/$ws" origin/main
```
...
```powershell
Set-Location ".worktrees/sprint-27-ws-a-corpus"
# Launch your Copilot CLI here, pasting the Section 6 prompt with WS = A.
```

## 5. Workstream -> worktree map

| WS | Branch (`sprint-27/...`) | Plan section | Wave | Depends on |
| -- | ------------------------ | ------------ | ---- | ---------- |
...
```text
You are implementing workstream <WS> of Sprint 27 (Curavias Product Owner Agent).

Read first (do not skip):
- Plan section WS-<WS> in docs/superpowers/plans/2026-07-25-sprint-27-product-owner-agent.md
- Design spec docs/superpowers/specs/2026-07-25-sprint-27-product-owner-agent-design.md
